...
/**
 * Custom Hook for Real-time Message Notifications
 * Polls for new messages and triggers toast notifications
 */
export const useMessageNotifications = (isEnabled = true) => {
  ...
  const loadMessages = useCallback(async () => {
    try {
      ...
      if (messageUnreadCount > previousMessageCountRef.current && previousMessageCountRef.current > 0) {
        ...
        setToastMessage(`${newCount} new ${newCount === 1 ? 'message' : 'messages'}`);
        setShowToast(true);
      }
